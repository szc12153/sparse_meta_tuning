...
class PetDataset(CombinationMetaDataset):
    def __init__(self, root, num_classes_per_task=None, meta_train=False,
                 meta_val=False, meta_test=False, meta_split=None,
                 transform=None, target_transform=None, dataset_transform=None,
                 class_augmentations=None, download=False):
        dataset = PetClassDataset(root, meta_train=meta_train,
                                   meta_val=meta_val, meta_test=meta_test, meta_split=meta_split,
                                   transform=transform, class_augmentations=class_augmentations,
                                   download=download)
        super(PetDataset, self).__init__(dataset, num_classes_per_task,
...
class PetClassDataset(ClassDataset):
    def __init__(self, root, meta_train=False, meta_val=False, meta_test=False,
                 meta_split=None, transform=None, class_augmentations=None,
                 download=False):
        super(PetClassDataset, self).__init__(meta_train=meta_train,
                                               meta_val=meta_val, meta_test=meta_test, meta_split=meta_split,
                                               class_augmentations=class_augmentations)
        self.root = root
        self.transform = transform
        temp_dataaset = OxfordIIITPet(
            root=self.root,
            split=meta_split,
            download=False
        )
        self.labels = np.unique(temp_dataaset._labels).tolist()
        assert len(self.labels) == 37
        self.data = []
        for c in self.labels:
            self.data.append([x for x,y in zip(temp_dataaset._images,temp_dataaset._labels) if y == c])
