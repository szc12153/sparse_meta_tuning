...
class FullMetaDatasetH5(torch.utils.data.Dataset):
    # todo； make output source_label realtive to the order of training domains sequence
    _dataset_to_idx = ["ilsvrc_2012","omniglot",'aircraft', 'cu_birds', 'dtd', 'quickdraw', 'fungi', 'vgg_flower','traffic_sign', 'mscoco']
    def __init__(self, args, split):
        ...
        self.return_source_info = args.return_source_info
        # self._dataset_to_idx = _dataset_to_idx
    ...
    def __getitem__(self, idx):
        ...
        if self.return_source_info:
            source_label = torch.tensor([FullMetaDatasetH5._dataset_to_idx.index(source)], device=support_images.device).reshape(1, )
