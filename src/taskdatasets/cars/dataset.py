...
class CARSDataset(CombinationMetaDataset):
    def __init__(self, root, num_classes_per_task=None, meta_train=False,
        ...
        dataset = CARSClassDataset(root, meta_train=meta_train,
            ...
            download=download)
        super(CARSDataset, self).__init__(dataset, num_classes_per_task,
...
class CARSClassDataset(ClassDataset):
    def __init__(self, root, meta_train=False, meta_val=False, meta_test=False,
        ...
        temp_dataaset = StanfordCars(
            root = self.root,
            split=meta_split,
            download=False
        )
        self.labels = np.unique([x[1] for x in temp_dataaset._samples]).tolist()
        assert len(self.labels)==196
        self.data = []
        for c in self.labels:
            self.data.append([x[0] for x in temp_dataaset._samples if x[1]==c])
